   ...
   <%try{
	 ...
	 session.setAttribute("date",date);
	    ...
	    // String mname=request.getParameter("mname");
	     ...
	     System.out.println("Connected"+session.getAttribute("mname")+tname+date+uname);
...
<td align="left"><label><%=date %></label></td>
 ...
 <tr>
       ...
       <%  while(rs2.next()){ %>
           ...
           <input type="radio" name="time" value="<%=rs2.getString(4) %>"/><%=rs2.getString(4) %>
           <input type="radio" name="time" value="<%=rs2.getString(5) %>"/><%=rs2.getString(5) %>
           <input type="radio" name="time" value="<%=rs2.getString(6) %>"/><%=rs2.getString(6) %>
           <input type="radio" name="time" value="<%=rs2.getString(7) %>"/><%=rs2.getString(7) %>
        <% } %></td>
